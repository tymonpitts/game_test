...
class AbstractTreeParent(AbstractTreeBase):
    ...
    def _init_column_from_height_map(self, values, indices, min_height, max_height, origin, info):
        ...
        #
        if min_ > max_height:
            self._children[indices[0]] = AbstractTreeLeaf(1)
            self._children[indices[1]] = AbstractTreeLeaf(1)
            return
        elif max_ <= min_height:
            self._children[indices[0]] = AbstractTreeLeaf(0)
            self._children[indices[1]] = AbstractTreeLeaf(0)
            return
        ...
        if max_ <= origin:
            ...
            self._children[indices[0]] = AbstractTreeInterior()
        ...
        if min_ > origin or all_leaf:
            ...
            self._children[indices[1]] = AbstractTreeInterior()
